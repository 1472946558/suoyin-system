...
> = {
  ...
  audit: {
    title: "操作日志",
    description: "角色调整、系统配置修改和风险提醒审计。",
    group: "系统与模板",
    ability: "audit.view",
    phase: "week1",
  },
...
const GROUP_ORDER = ["首页", "组织与权限", "商品与业务", "系统与模板"];
...
const PHASE_TWO_ITEMS: Record<PageId, string[]> = {
  ...
  orders: ["订单列表接口与详情抽屉", "按门店的数据范围隔离", "打印模板引用订单信息"],
  recycle: ["回收单状态流定义", "图片附件和留痕校验", "打印模板引用回收单"],
  "system-config": ["拍照规则字段清单", "编号规则和品牌配置", "系统配置修改审计"],
  "template-init": [],
  audit: ["高风险操作日志模型", "权限变更前后差异展示", "系统配置修改审计查询"],
};
...
const localDataEnabled = ADMIN_API_MODE !== "strict-api";
const runtimeModeLabel = computed(() => (localDataEnabled ? "离线数据已开启" : "正式接口模式"));
const runtimeModeBadgeTone = computed(() => (localDataEnabled ? "gold" : "emerald"));
const runtimeModeDescription = computed(() =>
  localDataEnabled
    ? "已显式开启离线数据。仅在登录或加载阶段且后端不可达时，可用于内部浏览；保存和新建仍只走真实 API，失败会直接返回接口错误。"
    : "当前为正式联调默认模式。登录、加载、保存、新建全部直连真实 API；失败不会自动伪装成功。",
);
const currentSourceLabel = computed(() => (sourceMode.value === "api" ? "当前数据来源：真实 API" : "当前数据来源：本地数据"));
const operatorAccountHint = computed(() =>
  localDataEnabled
    ? "备用账号仅用于内部浏览，不代表真实联调通过。"
    : "当前未启用离线数据。验收以真实 API 返回为准。",
);
...
async function loadConsole(token: string) {
  ...
  try {
    ...
    const nextError = error instanceof Error ? error : new Error("加载后台数据失败。");
    clearSession();
    loginError.value = nextError.message;
    throw nextError;
  } finally {
    loading.value = false;
  }
...
function selectOperatorAccount(username: string) {
...
async function handleSaveRole() {
  ...
  try {
    ...
    setNotice("success", `已通过真实 API 保存 ${result.data.name}。`);
  } catch (error) {
...
async function handleSaveStore() {
  ...
  try {
    ...
    setNotice("success", "门店资料已通过真实 API 保存。");
  } catch (error) {
...
async function handleSaveUser() {
  ...
  try {
    ...
    setNotice("success", "账号资料已通过真实 API 保存。");
  } catch (error) {
...
async function handleSaveProduct() {
  ...
  try {
    ...
    setNotice("success", "商品资料已通过真实 API 保存。");
  } catch (error) {
...
async function handleSaveSystem() {
  ...
  try {
    ...
    setNotice("success", "系统配置已通过真实 API 保存。");
  } catch (error) {
...
async function handleSavePrint() {
  ...
  try {
    ...
    setNotice("success", "打印模板已通过真实 API 保存。");
  } catch (error) {
...
async function handleCreateStore() {
  ...
  try {
    ...
    await loadConsole(sessionToken.value);
    selectedStoreId.value = result.data.id;
    setNotice("info", "已通过真实 API 创建门店草稿。");
  } catch (error) {
...
async function handleCreateUser() {
  ...
  try {
    ...
    await loadConsole(sessionToken.value);
    selectedUserId.value = result.data.id;
    setNotice("info", "已通过真实 API 创建账号草稿。");
  } catch (error) {
...
async function handleCreateProduct() {
  ...
  try {
    ...
    await loadConsole(sessionToken.value);
    selectedProductId.value = result.data.id;
    setNotice("info", "已通过真实 API 创建商品草稿。");
  } catch (error) {
  ...
  <div class="admin-app">
    <section v-if="!bootstrap" class="login-shell">
      <div class="login-hero panel">
        <p class="eyebrow">GOLD CASHIER ADMIN</p>
        <h1>黄金回收收银系统后台管理端</h1>
        <p class="hero-copy">
          聚焦登录、首页、门店管理、账号管理、角色权限、订单、回收留档和打印配置。
        </p>
        ...
        <div class="hero-grid">
          ...
          <article class="mini-card">
            <span>业务管理</span>
            <strong>订单 / 回收 / 留档</strong>
            <p>收银订单、回收单、照片留痕和打印准备。</p>
          </article>
          <article class="mini-card">
            <span>模板化交付</span>
            <strong>初始化入口</strong>
        ...
        <div class="login-info-box">
          <div>
            <h3>{{ runtimeModeLabel }}</h3>
            <p>{{ runtimeModeDescription }}</p>
          </div>
          <button type="button" class="secondary-btn" :disabled="!localDataEnabled" @click="selectOperatorAccount('boss')">
            填充老板备用账号
          </button>
          <button
            type="button"
            class="secondary-btn"
            :disabled="!localDataEnabled"
            @click="selectOperatorAccount('manager.sz')"
          >
            填充店长备用账号
          </button>
        </div>
      ...
      <div class="login-panel panel">
        <div class="panel-head">
          ...
          <span class="badge" :class="runtimeModeBadgeTone">{{ runtimeModeLabel }}</span>
        </div>
        ...
        <div class="login-tips">
          <p><strong>当前模式：</strong>{{ runtimeModeDescription }}</p>
          <p><strong>老板账号：</strong>`boss / Boss123!`，可查看全门店和角色权限页。</p>
          <p><strong>店长账号：</strong>`manager.sz / Manager123!`，默认只看所属门店和业务订单。</p>
          <p><strong>提示：</strong>{{ operatorAccountHint }}</p>
        </div>
      </div>
    </section>
    ...
    <div v-else class="workspace-shell">
      <aside class="sidebar panel">
        ...
        <div class="sidebar-note">
          ...
          <span>{{ runtimeModeDescription }}</span>
          <p class="mode-source">{{ currentSourceLabel }}</p>
        </div>
      </aside>
      ...
      <main class="workspace-main">
        <header class="workspace-header panel">
          ...
          <div class="header-actions">
            ...
            <span class="badge" :class="runtimeModeBadgeTone">
              {{ runtimeModeLabel }}
            </span>
            <span class="badge" :class="sourceMode === 'api' ? 'emerald' : 'gold'">
              {{ sourceMode === "api" ? "data: API" : "data: local" }}
            </span>
          ...
          <div class="content-grid two-up">
            <article class="panel">
              ...
              <div class="stack-list">
                <article v-for="task in dashboardData.todos" :key="task.id" class="list-card">
                  <div class="list-card-head">
                    <strong>{{ task.title }}</strong>
                    <span class="badge" :class="task.level === 'high' ? 'danger' : 'slate'">
                      {{ task.level === "high" ? "高优" : "处理中" }}
                    </span>
                  </div>
                  <p>{{ task.description }}</p>
                  <button type="button" class="text-btn" @click="openPage(task.page)">前往处理</button>
                </article>
              </div>
            </article>
            ...
            <article class="panel">
              ...
              <div class="stack-list">
                <article class="list-card">
                  <strong>对象存储</strong>
                  <p>{{ systemProfile?.ossStatus }}</p>
                </article>
                <article class="list-card">
                  <strong>打印设备</strong>
                  <p>{{ systemProfile?.printerStatus }}</p>
                </article>
            ...
            <div class="content-grid sidebar-layout">
              ...
              <aside v-if="storeDraft" class="detail-card panel">
                ...
                <div class="list-card">
                  ...
                  <p v-else>当前门店处于营业中，可继续核对人员归属、营业时间和打印准备口径。</p>
                </div>
        ...
        <section v-else-if="activePage === 'roles'" class="page-grid">
          ...
          <div v-if="roleDraft" class="content-grid two-up">
            ...
            <aside class="panel">
              ...
              <div class="list-card">
                <strong>风险提示</strong>
                <p>账号停用、权限变更、系统配置修改后续都应由后端写入操作日志。</p>
              </div>
            </aside>
          </div>
        </section>
        ...
        <section v-else-if="activePage === 'orders'" class="page-grid">
          <article class="panel">
            ...
            <div class="summary-row">
              ...
              <div class="summary-pill">已完成 {{ orderSummary.paid }} 笔</div>
              <div class="summary-pill">待复核 {{ orderSummary.pending }} 笔</div>
            ...
            <div class="content-grid sidebar-layout">
              ...
              <aside v-if="selectedOrder" class="detail-card panel">
                ...
                <div class="list-card">
                  <strong>处理建议</strong>
                  <p v-if="selectedOrder.status === 'pending'">当前订单仍待复核，建议核对门店、商品数与操作人留痕。</p>
                  <p v-else-if="selectedOrder.status === 'refunded'">当前订单已取消，建议核对门店口径与备注原因。</p>
                  <p v-else>当前订单已完成，可继续核对门店、商品数与操作人留痕是否完整。</p>
                </div>
              </aside>
            </div>
          </article>
            ...
            <div class="content-grid sidebar-layout">
              ...
              <aside v-if="selectedRecycleOrder" class="detail-card panel">
                ...
                <div class="list-card">
                  ...
                  <p v-else>当前回收单留痕基本完整，可继续联动打印模板验收。</p>
                </div>
              </aside>
            </div>
          </article>
        ...
        <section v-else-if="activePage === 'system-config' && systemDraft" class="page-grid">
          <article class="panel">
            <div class="panel-head">
              ...
              <span class="badge gold">业务要求优先 / ERP 参考补齐</span>
            </div>
          ...
          <article class="panel">
            ...
            <div class="content-grid sidebar-layout">
              ...
              <aside v-if="selectedAuditLog" class="detail-card panel">
                ...
                <div class="list-card">
                  <strong>处置建议</strong>
                  <p v-if="selectedAuditLog.riskLevel === 'high'">建议优先复核配置影响面，并同步门店或权限负责人。</p>
