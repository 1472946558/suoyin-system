...
func (s *MockStore) bootstrapPersistence() error {
	...
	customerAppointments, err := s.persistence.loadCustomerAppointments(ctx)
	if err != nil {
		return err
	}
	if len(customerAppointments) > 0 {
		s.customerAppointments = customerAppointments
	}
	s.customerSeq = nextSequenceFromCustomerAppointments(s.customerAppointments)
...
func (s *MockStore) bootstrapBaseConfigs(ctx context.Context) error {
	...
	found, err = s.persistence.loadConfig(ctx, configKeyCustomerProfiles, &customerProfiles)
	if err != nil {
		return err
	}
	if found {
		s.customerProfiles = customerProfiles
	} else if err := s.persistence.saveConfig(ctx, configKeyCustomerProfiles, s.customerProfiles); err != nil {
		return err
	}

	var homeConfig CustomerHomeResponse
	found, err = s.persistence.loadConfig(ctx, configKeyCustomerHome, &homeConfig)
	if err != nil {
		return err
	}
	if found {
	...
	found, err = s.persistence.loadConfig(ctx, configKeyCustomerRecycleInfo, &recycleInfo)
	if err != nil {
		return err
	}
	if found {
		s.customerRecycleInfo = recycleInfo
	} else {
		s.customerRecycleInfo = defaultCustomerRecycleInfo()
		if err := s.persistence.saveConfig(ctx, configKeyCustomerRecycleInfo, s.customerRecycleInfo); err != nil {
			return err
		}
	}
