...
## 2026-08-17 功能点 06：首页轮播与首页入口反馈修复

### 本次交付范围
- 首页使用真实 `swiper` 轮播配置：自动播放、循环、切换动画、指示点；后台有 Banner 图片时优先展示后台配置。
- 当生产首页 Banner 配置为空时，前端提供 3 张可点击的默认轮播内容，避免首页出现空白轮播区域；后续后台补充图片后自动替换。
- 为“点击查看”、门店详情、到店预约、款式页、门店页和地图打开增加失败反馈，避免点击后无任何可见状态。
- 地图导航增加经纬度有效性判断；门店坐标为 `0,0` 时明确显示“暂无坐标”，并提示后台补充门店经度和纬度。

### 现网只读核验结论
- `GET /api/v1/customer/home` 返回 `banners: []`：轮播空白的直接原因是后台没有启用 Banner 数据，不是 swiper 未实现。
- `GET /api/v1/customer/stores/store-new-010` 返回 `longitude: 0`、`latitude: 0`：地图按钮无法打开真实位置的直接原因是门店坐标缺失。
- 商品列表接口、门店预约时段接口均返回正常数据；“点击查看”和“到店预约”已在微信开发者工具中实际进入对应页面。

### 验收证据
- 顾客端全部 JavaScript `node --check`：通过。
- 顾客端全部 JSON 解析：通过。
- `miniapp/npm run validate`：通过。
- `git diff --check`：通过。
- 微信开发者工具普通编译：首页显示 3 张默认轮播，自动切换和指示点可见；“点击查看”进入款式页；“到店预约”进入预约创建页；无坐标门店显示“暂无坐标”。
- 开发者工具未发现业务运行时错误；控制台仅有微信运行时不支持 `reportRealtimeAction` 的平台告警，不涉及本次业务逻辑。

### 交付边界
- 本次只修改顾客首页前端和项目变更记录；未修改生产数据库、生产服务器、后台 Banner 数据，未执行微信上传或发布。
- 生产可用前，后台需要配置启用 Banner 图片，并为可导航门店填写有效经纬度；这两项属于运营/后台数据准备，不应通过前端伪造。
...
## 2026-08-17 功能逐项调试与根因修复（当前工作区，未部署）

### 后端

- 预约创建、取消、顾客备注、员工状态、后台取消和后台备注统一经过持久化出口；写库失败会恢复内存状态并返回 50302，不再吞掉错误。
- 保留并验证预约死锁修复：规则读取使用锁内快照；容量错误映射为 40904；未验证手机号映射为 40310。
- persistent 模式启动时拒绝默认密钥、wildcard CORS、mock login 和缺少微信凭据；持久化 Compose 模板将 `TOKEN_SECRET`、`CORS_ORIGIN` 设为必填。
- 新增 CORS 与 persistent 配置测试。

### 顾客端

- 首页 Banner 继续采用后台数据优先、fallback 兜底；轮播和首页卡片补充路由失败反馈。
- 门店、首页和门店详情统一校验非零数字坐标；无坐标时显示禁用态，不调用错误地图导航。
- 门店/预约路由参数编码，路由失败显示 toast；店长入口继续从顾客“我的”内部进入员工登录页。

### 验证

- 微信开发者工具：轮播、门店、预约、我的、店长入口关键路径已逐项回归；问题计数 0。
- `go test ./...`、预约/CORS focused `go test -race`、`go vet ./...`、小程序 validate、管理后台 build、JS/JSON 语法检查、`git diff --check` 均通过。
- 生产公网仅做只读检查：Banner 为空、门店坐标为 0、CORS 为 `*`、HTTP 未跳 HTTPS、一个静态资源返回 404；未部署、未修改生产数据库或服务器。

### 当前结论

- 代码状态：交付候选。
- 商用状态：未通过。生产数据补齐、安全配置部署、真机授权、员工隔离和备份恢复仍是外部验收闸门。
