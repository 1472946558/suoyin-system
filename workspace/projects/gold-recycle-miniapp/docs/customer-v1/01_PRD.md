...
在现有金匠倌员工/门店管理小程序中，新增面向普通顾客的功能模块。同一个小程序、同一 AppID，默认进入顾客端，员工从"我的→店长入口"登录。
...
| 页面 | 路径 | 权限 |
|------|------|------|
| 店长登录 | pages/account/index | guest |
...
### 6.3 店长登录流程
```
我的 → 店长入口 → 账号密码登录 → 后端校验角色权限 → 进入工作台/管理端
```
